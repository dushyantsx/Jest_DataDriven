...
const testId = TestIdsMap.api1test;
...
let caseResult;
const testName =
  "Provide SHS API Test : Provide SHS; Product: Home Security 60 Months; Techniocian Install";
describe(testName, () => {
  let steps;
  ...
  const step2CreateShsCommitmentLw = async (datasetIndex, stepNum) => {
    ...
    const offerList = [secureOffer, commitmentOffer];
    ...
    return btapi
      ...
      .then(
        (success) => {
          ...
          logger.info(`CreateSC API response:${responseText}`);
          ...
          body.cartItem.forEach(function (cartItem) {
            if (cartItem.productOffering === secureItem) {
              btapi.getByParent("id", secureOffer, body.cartItem);
  ...
  const step5Appointments = async (datasetIndex, stepNum) => {
    ...
    logger.info(
      `Step05: search availableappointment API response:${JSON.stringify(
        response
      )}`
    );
    let appointmentList = [];
  ...
  const step6UpdateWoSo = async (datasetIndex, stepNum) => {
    ...
    logger.info(`Step06: Update WO and SO body:${JSON.stringify(body)}`);
    ...
    return btapi
      ...
      .then(
        (success) => {
          ...
          logger.info(
            `Step06: Update WO and SO API response:${JSON.stringify(success)}`
          );
          expect(
            success.response.body,
            `Response should contain body\n${JSON.stringify(
  ...
  const step6ValidateSc = async (datasetIndex, stepNum) => {
    ...
    logger.info(`Step 6 validate SC body:${JSON.stringify(body)}`);
    const result = {};
    result.reqBody = body;
    return btapi
      .$requestShoppingCart(
      ...
      .then(
        (success) => {
          ...
          logger.info(`Validate SC response:${JSON.stringify(success)}`);
          expect(
            success.response,
            `Response field should be present\n${JSON.stringify(
              success,
    ...
    return btapi
      ...
      .then(
        (success) => {
          ...
          expect(
            secureItem,
            `Security offer (${secureItem}) should be present in response\n${scText}`
          ).not.toBeNull();
          const commitmentItem = btapi.getByParent(
            "id",
            commitmentOffer,
  ...
  const addequipments = async (datasetIndex, stepNum) => {
    ...
    const externalLocationId =
      caseResult.datasets[datasetIndex].response[0].result.reqBody
        .externalLocationId;
    const salesOrderId =
      caseResult.datasets[datasetIndex].response[7].result.SaleOrderId;
    ...
    const childOfferList = [btapi.data.homeSecurityEquipments.LawnSign];
    ...
    return btapi
      ...
      .then(
        (success) => {
          ...
          expect(
            secureItemId,
            `secure offer (${secureOffer}) should be present in response as 1 equipment were added.\n${scText}`
          ).not.toBeNull();
          body.cartItem.forEach(function (item) {
          ...
          const expChildren = [btapi.data.homeSecurityEquipments.LawnSign];
          const actChildren = [];
          secureItem.cartItem.forEach(function (childItem) {
            const childProduct = childItem.productOffering;
            actChildren.push(childProduct.id);
  ...
  const removeTopofferstocease = async (datasetIndex, stepNum) => {
    ...
    const offerItemList = [secureItemId];
    logger.info(`secureitem${secureItemId}`);
    ...
    return btapi
      ...
      .then(
        (success) => {
          ...
          expect(
            secureItemId,
            `secure offer (${secureOffer}) should be present in response\n${scText}`
          ).not.toBeNull();
          body.cartItem.forEach(function (item) {
            const product = item.product;
            expect(
