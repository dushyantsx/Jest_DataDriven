...
async function stepsProvide1LWself(dsObj, datasetindex) {
  ...
  try {
    ...
    let response = await btapi.verifyCreateCustomerAccountTBAPI(body).then(
      (success) => {
        expect(
          success,
        ...
        customerAccountECID = response.success.externalCustomerId;
      },
      (error) => {
        expect(
          true,
    ...
    response = await btapi
      ...
      .then(
        (success) => {
          ...
          responseText = JSON.stringify(success, null, "\t");
          expect(
    ...
    const distributionChannelName = "CSR";
