...
describe("SHS and LW Test Scenarios", () => {
  ...
  const testName = "Ammend-1: Add Equipment";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Ammend-1");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Ammend-1");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsAmmend1(dsObj, index)
            .then((v) => {
...
async function stepsAmmend1(dsObj, datasetindex) {
