...
const testId = TestIdsMap.ammend2TestID;
...
describe("SHS and LW Test Scenarios", () => {
  ...
  const testName = "Ammend-2: Add Equipment";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Ammend-2");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Ammend-2");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsAmmend2(dsObj, index)
            .then((v) => {
...
async function stepsAmmend2(dsObj, datasetindex) {
