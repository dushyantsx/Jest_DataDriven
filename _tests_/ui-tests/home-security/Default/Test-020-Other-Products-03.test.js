...
describe("SHS and LW Test Scenarios", () => {
  ...
  const testName =
    "Other-Products-03: HS and Optik TV to already existing connection";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Other-Products-03");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Other-Products-03");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsOther3(dsObj, index)
            .then((v) => {
...
async function stepsOther3(dsObj, datasetindex) {
  ...
  try {
    ...
    let availsAndSelected = await SelectServicesPage.selectTVPlan(
      tvservice,
      tvcommitments,
      tvproducttype,
      tvproductpackage,
    ...
    const tvequipments = await EquipmentPage.completeHomeSecuritySectionOnDefaults();
