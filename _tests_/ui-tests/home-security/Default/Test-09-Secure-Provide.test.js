...
describe("SHS and LW Test Scenarios", () => {
  ...
  const testName = "ProvideSecure-HS for 36 Months";
  describe(testName, () => {
    beforeAll(() => {
  ...
  try {
    ...
    if (isValEnabled) {
      ...
      const cityDb = await du.getValue(
        dbcfg,
        dq.queryNcObjectNameOnId(output.city.itemid)
      );
      ...
      const addressDb = await du.getValue(
        dbcfg,
        dq.queryCompleteAddress(output.address.rel.id)
      );
    ...
    const customerId = await du.getValue(
      dbcfg,
      dq.queryNcCustomerIdFromSaleOrderNumber(output.orderNumber)
    );
