...
const testId = TestIdsMap.provide12SHSLWNB;
 ...
 */
let caseResult;
describe("Provide12: SC LW Test Scenarios Provide", () => {
  beforeAll(() => {
  ...
  const testName =
    "Provide-12: Provide in NB; Product: SSH Home security and Living well; Self Install";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Provide-12");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Provide-12");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsProvide12SHLW(dsObj, index)
            .then((v) => {
...
async function stepsProvide12SHLW(dsObj, datasetindex) {
