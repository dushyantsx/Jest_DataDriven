...
const testId = TestIdsMap.provide1lwselfTestId;
 ...
 */
let caseResult;
describe("Provide1: Provide in ON; Product: LivingWell Companion Go, Self-Install derlivery method", () => {
  beforeAll(() => {
  ...
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll stepsProvide1LWself");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll stepsProvide1LWself");
    });
...
async function stepsProvide1LWself(dsObj, datasetindex) {
  ...
  try {
    ...
    expect(JSON.stringify(popupcontentdelivery.msgtext)).toContain(msgtoverify);

    await EquipmentPage.selectServicePlanTab("LivingWell");
    ...
    const popupcontent = await RulesPopup.acceptRulesSafely();
    expect(popupcontentdelivery).not.toBeNull();
    expect(popupcontentdelivery).not.toBeUndefined();
    const msgtoverifyaddon =
      "You’ve chosen Self-Install and LWC Direct Ship has been added to your order.";
    expect(JSON.stringify(popupcontent.msgtext)).toContain(msgtoverifyaddon);
    ...
    if (isValEnabled) {
      ...
      expect(JSON.stringify(tax.appliedtax)).toContain(taxToValidate);
    }
    ...
    await ShippingDetailsPage.shipToPreFilledAddressAndSubmit(
    ...
    try {
      ...
      //eating error if page is not displayed
    }
