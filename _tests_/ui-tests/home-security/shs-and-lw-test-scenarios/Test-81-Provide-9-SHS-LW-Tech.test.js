...
const testId = TestIdsMap.provide9SSHLWTech;
 ...
 */
let caseResult;
describe("Provide9: SC LW Test Scenarios", () => {
  beforeAll(() => {
  ...
  const testName =
    "Provide-9: Provide in SASK; Product: SSH Home security and Living well; Self Install";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Provide-9");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Provide-9");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsProvide9(dsObj, index)
            .then((v) => {
...
async function stepsProvide9(dsObj, datasetindex) {
