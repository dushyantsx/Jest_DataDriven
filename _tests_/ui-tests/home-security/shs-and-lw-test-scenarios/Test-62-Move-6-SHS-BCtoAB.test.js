...
const testId = TestIdsMap.move6TestId;
 ...
 */
let caseResult;
describe("Move6: SHS From BC to AB with package downgrade", () => {
  beforeAll(() => {
  ...
  const testName = "Move-6: SHS From BC to AB with package downgrade";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Move-6");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Move-6");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsMove6(dsObj, index)
            .then((v) => {
...
async function stepsMove6(dsObj, datasetindex) {
  ...
  try {
    ...
    const provide7email = config.getFirstTestDataAssetResultsForGivenTestId(
      TestIdsMap.provide7SSHTech
    ).request.email;

    await CsrDesktopPage.searchCustomerForEmail(provide7email);
    ...
    SelectServicesPage.clickSSHViewMore();
    ...
    const equipments = await EquipmentPage.completeYouPickSectionOnDefaults();
    ...
    if (isValEnabled) {
      ...
      expect(equipments.ypEquipments).not.toBeNull();
      expect(equipments.ypEquipments).not.toBeUndefined();
    }
    ...
    let adcresponse = await adcapis.isCustomerAvailable(adcfg, provide7email);
