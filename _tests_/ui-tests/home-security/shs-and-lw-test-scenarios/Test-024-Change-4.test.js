...
describe("SHS and LW Test Scenarios", () => {
  ...
  let testName = "Change-4: Downgrade Package";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsChange4(dsObj, datasetindex) {
  ...
  try {
    ...
    await ManageServicesPage.clickChangeonService("Secure");

    await SelectServicesPage.selectCommitments(dr.getDataSetKeyValue(dsObj, "input", "commitments"));

    await SelectServicesPage.selectPlan(dr.getDataSetKeyValue(dsObj, "input", "serviceplan"));
    ...
    if (StringUtils.containsIgnoreCase(sPlan, "secure") == false) {
      var youpickequipment = await EquipmentPage.completeYouPickSectionOnDefaults();
      storeOutputToDataSetResult(caseResult, datasetindex, youpickequipment);

      logger.result(`Default equipments are selected `);
    }

    let homesecurityequipment = await EquipmentPage.completeHomeSecuritySectionOnDefaults();
    storeOutputToDataSetResult(caseResult, datasetindex, homesecurityequipment);

    let addonequipment = await EquipmentPage.completeAddOnEquipmentSectionOnDefaults();
    storeOutputToDataSetResult(caseResult, datasetindex, addonequipment);

    RulesPopup.clickOnContinueWithOrder();
