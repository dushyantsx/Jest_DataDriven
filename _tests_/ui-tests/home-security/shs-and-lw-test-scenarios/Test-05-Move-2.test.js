 ...
 */
let caseResult;
describe("Move2: SHS & LW Test Scenarios", () => {
  beforeAll(() => {
  ...
  let testName = "Move-2: Negative test";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsMove2(dsObj, datasetindex) {
  ...
  try {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", testId, "validate-available-services");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", testId, "validate-default-equipments");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", testId, "validate-no-easy-payments");
    if (isValEnabled) {
    ...
    let noAddOnAdditionalEquipments = await EquipmentPage.completeAddOnEquipmentSectionOnDefaults(deliveryMethod, true);
    storeOutputToDataSetResult(caseResult, datasetindex, noAddOnAdditionalEquipments);
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", testId, "validate-no-additional-equipment");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", testId, "validate-tax-rate");
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", testId, "validate-cost");
    if (isValEnabled) {
