...
describe("SHS and LW Test Scenarios", () => {
  ...
  let testName = "Change-6: Book Appointment";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsChange6(dsObj, datasetindex) {
  ...
  try {
    ...
    await AppointmentPage.selectServiceForAppointment("Secure");

    await AppointmentPage.selectAvailableDate();

    await AppointmentPage.selectAvailableTime();

    await AppointmentPage.enterContactDetails(
      dr.getDataSetKeyValue(dsObj, "input", "firstname"),
      dr.getDataSetKeyValue(dsObj, "input", "contactphonenumber")
    );

    await AppointmentPage.clickConfirmButton();
