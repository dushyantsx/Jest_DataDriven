...
describe("Provide3: SHS & LW Test Scenarios", () => {
  ...
  let testName = "Provide-3: Multi location Customer";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsProvide3(dsObj, datasetindex) {
  ...
  try {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-2", "validate-address");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-2", "validate-customer-registered");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-1", "validate-available-services");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-1", "validate-default-equipments");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-1", "validate-no-easy-payments");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-2", "validate-tax-rate");
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-2", "validate-cost");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-2", "validate-order-submission");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-2", "validate-be-time-zone");
    if (isValEnabled) {
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-2", "validate-services-active");
    if (isValEnabled) {
    ...
    await AccountPage.selectAddress(dr.getDataSetKeyValue(dsObj, "input", "expectedaddress"));

    await AccountPage.clickOnAddService(dr.getDataSetKeyValue(dsObj, "input", "secondservice"));

    availableAndSelected = await SelectServicesPage.selectServicesVerticalPlanProvider(
    ...
    let deliveryMethod2 = "Self";
    await EquipmentPage.completeLivingWellSectionOnDefaults(deliveryMethod2);
    let noAdditionalEquipments2 = await EquipmentPage.completeAddOnEquipmentSectionOnDefaults(deliveryMethod2, true);
    logger.result(`No option to add any additional equipment: ${noAdditionalEquipments2}`);

    storeOutputToDataSetResult(caseResult, datasetindex, noAdditionalEquipments2);
    isValEnabled = tval.isValidationEnabled(
      "shs-and-lw-test-scenarios",
      "provide-3",
      "validate-no-additional-equipment"
    ...
    if (isValEnabled) {
      logger.result(`No option to add any additional equipment`);
      expect(noAddOnAdditionalEquipments2).not.toBeNull();
      expect(noAddOnAdditionalEquipments2).not.toBeUndefined();
      expect(noAddOnAdditionalEquipments2).toBeTruthy();
    }

    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-3", "validate-cost");
    if (isValEnabled) {
      let cost2 = await CheckoutPage.getDueMonthlyBeforeTaxMatching();
      let costToValidate2 = dr.getDataSetKeyValue(dsObj, "expected", "costtovalidate");
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-1", "validate-order-submission");
    if (isValEnabled) {
      logger.debug(`Validating order submission by getting order number generated from Agent portal`);
      expect(output).not.toBeNull();
      expect(output).not.toBeUndefined();
    ...
    ordersStatuses = {};
    logger.debug("Fetching customer's internal id");
    customerId = await du.getValue(dbcfg, dq.queryNcCustomerIdFromSaleOrderNumber(dbcfg,output.orderNumber));
    logger.debug(`Customer's internal id: ${customerId}`);
    ordersStatuses.customerId = customerId;
    logger.debug(`Order's statuses till now: ${JSON.stringify(ordersStatuses)}`);
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-1", "validate-be-time-zone");
    if (isValEnabled) {
      logger.debug(`Validating no error occured on NetCracker BE for timezone of ${output.orderNumber}`);
      customerErrors = await du.getErrorsOccuredForCustomer(dbcfg, customerId);
      expect(customerErrors).not.toBeNull();
      expect(customerErrors).not.toBeUndefined();
      expect(customerErrors.length > 0).toBeFalsy();
    }
    ...
    logger.debug("Fetching customer's all order item's status");
    allOrdersStatus = await du.select(dbcfg, dq.queryNcCustomerOrdersStatus(dbcfg,customerId));
    logger.debug(`Orders' statuses: ${JSON.stringify(allOrdersStatus)}`);
    ordersStatuses.allOrdersStatus = allOrdersStatus;
    logger.debug(`Order's statuses till now: ${JSON.stringify(ordersStatuses)}`);

    logger.debug("Fetching customer's all pending order item's status");
    allPendingOrders = await du.select(dbcfg, dq.queryNcCustomerOrdersStatusNeitherCompletedNorProcessed(customerId));
    logger.debug(`Pending Orders' statuses: ${JSON.stringify(allPendingOrders)}`);
    ordersStatuses.allPendingOrders = allPendingOrders;
    logger.debug(`Order's statuses till now: ${JSON.stringify(ordersStatuses)}`);
    storeOutputToDataSetResult(caseResult, datasetindex, ordersStatuses);
    ...
    if (allPendingOrders != null && allPendingOrders !== undefined && allPendingOrders.length > 0) {
      for (orIndex = 0; orIndex < allPendingOrders.length; orIndex++) {
        orderInternalId = allPendingOrders[orIndex][1];
        orderName = allPendingOrders[orIndex][0];
        if (StringUtils.containsIgnoreCase(orderName, "shipment")) {
          // Hit release activation in case order is in entering state
          await tapis.processReleaseActivation(apicfg, orderInternalId);
          // Wait for 10 seconds to get completed
          ...
          res = await du.getShipmentOrderNumberAndPurchaseOrderNumber(dbcfg, orderInternalId);
          // Hit shipment order completion
          await tapis.processShipmentOrder(apicfg, res.shipmentOrderNumber, res.purchaseeOrderNumber);
          // Wait for 10 seconds to get completed
          await sel.getWaitUtils().sleep(10000);
    ...
    isValEnabled = tval.isValidationEnabled("shs-and-lw-test-scenarios", "provide-1", "validate-services-active");
    if (isValEnabled) {
      logger.debug(`Validating service [${service}] is active or not`);
      ...
      await CsrDesktopPage.clickCityRadioButton(dr.getDataSetKeyValue(dsObj, "input", "secondcity"));

      await CsrDesktopPage.clickOnServiceslink();
      let servicePlan = dr.getDataSetKeyValue(dsObj, "input", "secondserviceplan");
      let isActive = await AccountPage.validateStatusIsActiveForGivenService(servicePlan);
