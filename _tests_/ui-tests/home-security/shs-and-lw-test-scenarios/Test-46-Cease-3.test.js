...
const testId = TestIdsMap.cease3TestId;
 ...
 */
let caseResult;
describe("Cease3: LW Future Dated Cease", () => {
  beforeAll(() => {
  ...
  const testName = "Cease-3: Future Date Cease";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Cease-3");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Cease-3");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsCease3(dsObj, index)
            .then((v) => {
...
async function stepsCease3(dsObj, datasetindex) {
  ...
  try {
    ...
    const move4serv = config.getFirstTestDataAssetResultsForGivenTestId(
      TestIdsMap.move4TestId
    ).request.serviceplan;
    ...
    await ManageServicesPage.checkServiceCheckbox(move4serv);
