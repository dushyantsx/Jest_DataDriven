...
const testId = TestIdsMap.provide13SHSLWPEI;
 ...
 */
let caseResult;
describe("Provide13 SC LW Test Scenarios Provide in PEI", () => {
  beforeAll(() => {
  ...
  const testName =
    "Provide-13: Provide in PEI; Product: SSH Home security and Living well; Self Install";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Provide-13");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Provide-13");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsProvide13SCLW(dsObj, index)
            .then((v) => {
...
async function stepsProvide13SCLW(dsObj, datasetindex) {
