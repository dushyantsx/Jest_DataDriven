...
describe("Provide1: SHS & LW Test Scenarios", () => {
  ...
  let testName = "Provide-1: Provide in ON; Product: LivingWell Companion Go; Self Install";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsProvide1(dsObj, datasetindex) {
  ...
  try {
    ...
    await sel.getWaitUtils().sleep(25000);
    ...
    if (isValEnabled) {
      ...
      let cityDb = await du.getValue(dbcfg, dq.queryNcObjectNameOnId(dbcfg, output.city.itemid));
      ...
      let addressDb = await du.getValue(dbcfg, dq.queryCompleteAddress(dbcfg, output.address.rel.id));
      ...
      storeOutputToDataSetResult(caseResult, datasetindex, cityDb);
      logger.info();
    }
    ...
    let availsAndSelected = await SelectServicesPage.selectServicesVerticalPlanProvider(
    ...
    if (isValEnabled) {
      ...
      let expectedPrice = dr.getDataSetKeyValue(dsObj, "expected", "costtovalidate");
    ...
    if (isValEnabled) {
      ...
      storeOutputToDataSetResult(caseResult, datasetindex, invisible);
    }
    ...
    if (StringUtils.equalsIgnoreCase(prv, "ON")) {
      ...
      var popupdetail = [];
      popupcontentdelivery = await RulesPopup.acceptRulesSafely();
      expect(popupcontentdelivery).not.toBeNull();
      expect(popupcontentdelivery).not.toBeUndefined();
      let msgtoverify = "The selected delivery method is not supported, please choose another";
      expect(popupcontentdelivery.msgtext).toContain(msgtoverify);
      //  popupdetail.push(popupcontent);

      storeOutputToDataSetResult(caseResult, datasetindex, popupcontentdelivery);
      await EquipmentPage.selectServicePlanTab(dr.getDataSetKeyValue(dsObj, "input", "services"));
    }
    ...
    // Tech Install validation steps
    {
      ...
      let equipments = await EquipmentPage.completeLivingWellSectionOnDefaults(deliveryMethod);
      ...
      if (isValEnabled) {
        ...
        expect(StringUtils.replaceAll(JSON.stringify(equipments.lwincludedEquipments), "[^a-zA-Z_]", "")).toContain(
        ...
        let allAddiontalEquipment = dr.getDataSetKeyValue(dsObj, "expected", "AdditionalEquipment");
        expect(
          StringUtils.replaceAll(JSON.stringify(equipments.lwlwadditionalEquipments), "[^a-zA-Z0-9]", "")
        ).toContain(StringUtils.replaceAll(allAddiontalEquipment, "[^a-zA-Z]", ""));
      }

      let addOnAdditionalEquipments = await EquipmentPage.completeAddOnEquipmentSectionOnDefaults(deliveryMethod);
      ...
      if (isValEnabled) {
        ...
        expect(addOnAdditionalEquipments.additionalEquipments).not.toBeUndefined();
        let exAddOnEquipment = dr.getDataSetKeyValue(dsObj, "expected", "AddOnEquipment");
        expect(
          StringUtils.replaceAll(JSON.stringify(addOnAdditionalEquipments.additionalEquipments), "[^a-zA-Z0-9]", "")
        ).toContain(StringUtils.replaceAll(exAddOnEquipment, "[^a-zA-Z0-9]", ""));
        expect(addOnAdditionalEquipments.popup).not.toBeNull();
        expect(addOnAdditionalEquipments.popup).not.toBeUndefined();

        let msgtoverify = "You’ve chosen Self-Install and LWC Direct Ship has been added to your order.";
    ...
    await CheckoutPage.applyPromoCode(dr.getDataSetKeyValue(dsObj, "input", "couponcode"));
    ...
    let discount = await CheckoutPage.getDiscount();

    storeOutputToDataSetResult(caseResult, datasetindex, discount);
    isValEnabled = tval.isValidationEnabled(valFile, testId, "Discount");
    if (isValEnabled) {
      let discounttovalidate = dr.getDataSetKeyValue(dsObj, "expected", "discount");
      logger.debug(`Validating cost ${discount.discountamount} with ${discounttovalidate}`);
      expect(StringUtils.equalsIgnoreCase(discount.discountamount, discounttovalidate)).toBeTruthy();
    }

    let totaltax = await CheckoutPage.getEstimatedTotalTaxesforToday();
    storeOutputToDataSetResult(caseResult, datasetindex, totaltax);

    let totaltopaytoday = await CheckoutPage.getEstimatedTotalToPayToday();
    storeOutputToDataSetResult(caseResult, datasetindex, totaltopaytoday);
    ...
    storeOutputToDataSetResult(caseResult, datasetindex, temp);
    ...
    if (allPendingOrders != null && allPendingOrders !== undefined && allPendingOrders.length > 0) {
      for (let orIndex = 0; orIndex < allPendingOrders.length; orIndex++) {
        ...
        if (StringUtils.containsIgnoreCase(orderName, "shipment")) {
          ...
          storeOutputToDataSetResult(caseResult, datasetindex, res);
          // Hit shipment order completion
          let orderstatus = await tapis.processShipmentOrder(apicfg, res.shipmentOrderNumber, res.purchaseeOrderNumber);
          storeOutputToDataSetResult(caseResult, datasetindex, orderstatus);
          // Wait for 10 seconds to get completed
          await sel.getWaitUtils().sleep(10000);
        }
      }
    ...
    isValEnabled = tval.isValidationEnabled(valFile, testId, "validate-billing-task");
    if (isValEnabled) {
      logger.debug(`Validating billing actions occured on NetCracker for customer ${customerId}`);
      let billingActionsStatus = await du.getBillingActionStatus(dbcfg, customerId);
      storeOutputToDataSetResult(caseResult, datasetindex, billingActionsStatus);
      ...
      logger.debug(`Validating no error occured on NetCracker for any billing task for customer ${customerId}`);
      billingActionsStatus = await du.getBillingFailedActionStatus(dbcfg, customerId);
      storeOutputToDataSetResult(caseResult, datasetindex, billingActionsStatus);
      expect(billingActionsStatus).not.toBeNull();
      expect(billingActionsStatus).not.toBeUndefined();
