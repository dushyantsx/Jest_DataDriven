...
const testId = TestIdsMap.move5TestID;
 ...
 */
let caseResult;
describe("Move5: SHS From AB to BC with same package", () => {
  beforeAll(() => {
  ...
  const testName = "Move5: SHS From AB to BC with same package";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Move-5");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Move-5");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsMove5(dsObj, index)
            .then((v) => {
...
async function stepsMove5(dsObj, datasetindex) {
  ...
  try {
    ...
    const availsAndSelected = await SelectServicesPage.getServiceDetails();
    ...
    let adcresponse = await adcapis.isCustomerAvailable(adcfg, provide6email);
