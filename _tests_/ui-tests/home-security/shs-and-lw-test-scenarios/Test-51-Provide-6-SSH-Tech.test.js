...
const { CreditApprovalPage } = require("../../../../src/pages/home-security");
...
const testId = TestIdsMap.provide6TestID;
 ...
 */
let caseResult;
describe("Provide6: SHS Tech Install and Additional Equipments (Easy Pay) ", () => {
  beforeAll(() => {
  ...
  const testName =
    "Provide-6:  SHS Tech Install and Additional Equipments (Easy Pay)";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Provide-6");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Provide-6");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsProvide6SSH(dsObj, index)
            .then((v) => {
...
async function stepsProvide6SSH(dsObj, datasetindex) {
  ...
  try {
    ...
    await EquipmentPage.clickOnAddEquipment(
      dr.getDataSetKeyValue(dsObj, "input", "YourPickCamera1")
    );
    ...
    await EquipmentPage.equipDD(
      dr.getDataSetKeyValue(dsObj, "input", "YourPickCamera2")
    );

    await EquipmentPage.selectTelusEasyPayment(
      dr.getDataSetKeyValue(dsObj, "input", "YourPickCamera2")
    );

    await EquipmentPage.clickOnAddEquipmentViaEasyPayment(
      dr.getDataSetKeyValue(dsObj, "input", "YourPickCamera2")
    );
    await EquipmentPage.clickOnAddEquipmentViaEasyPayment(
      dr.getDataSetKeyValue(dsObj, "input", "YourPickCamera2")
    ...
    await EquipmentPage.cameraMaxCountMsg();

    await sel.getJsUtils().isPageLoaded();

    const deliveryMethod = await EquipmentPage.selectDeliveryMethod(
      dr.getDataSetKeyValue(dsObj, "input", "delivery")
    );
    TestResult.storeOutputToDataSetResult(
      caseResult,
      datasetindex,
    ...
    await EquipmentPage.clickConfirmTakeOver();
    ...
    const msg =
      "You’ve chosen Installer Supplied as your shipping method so the Getting Started kit has been removed from your order An Installation charge of $450 is applicable on this order. Do you want to continue?";

    //const msgtext ="An Installation charge of $450 is applicable on this order. Do you want to continue?";

    try {
      const msg2 = await RulesPopup.clickonMultiplepopups("2");
      expect(
        StringUtils.containsIgnoreCase(msg, JSON.stringify(msg2.text))
      ).toBeTruthy();
    } catch (err) {
    ...
    const availableAppointment = await AppointmentPage.clickonSelectedService(
      dr.getDataSetKeyValue(dsObj, "input", "serviceplan")
    );

    // const availableAppointment = await AppointmentPage.submitAnyAvailableAppointment(
    //   dr.getDataSetKeyValue(dsObj, "input", "firstname"),
    //   dr.getDataSetKeyValue(dsObj, "input", "contactphonenumber"),
    //   "TestCase6"
    // );
    ...
    logger.result(
      `Select available appointment ${JSON.stringify(availableAppointment)}`
    );

    // expect(availableAppointment.swtime).toContain(
    //   dr.getDataSetKeyValue(dsObj, "expected", "swt")
    // );
    ...
    await sel.getWaitUtils().waitForUrlToChangeTo("billing");
    ...
    //Code Commented can be used in future for adc and dst validation when appointments are available for eastern provinces
