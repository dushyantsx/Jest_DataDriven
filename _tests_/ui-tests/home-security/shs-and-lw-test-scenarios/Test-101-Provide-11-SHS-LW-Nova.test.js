...
const testId = TestIdsMap.provide11SHSLWNova;
 ...
 */
let caseResult;
describe("Provide11: SC LW Test Scenarios", () => {
  beforeAll(() => {
  ...
  const testName =
    "Provide-11: Provide in NOVA; Product: SSH Home security and Living well; Self Install";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Provide-11");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Provide-11");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsProvide11(dsObj, index)
            .then((v) => {
...
async function stepsProvide11(dsObj, datasetindex) {
