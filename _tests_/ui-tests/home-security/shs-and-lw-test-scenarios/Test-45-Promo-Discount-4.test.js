...
const testId = TestIdsMap.promo4TestId;
 ...
 */
let caseResult;
describe("PROMO4: Promo/Discount-4 Provide SHS BC add promo code (negative test) Equip60", () => {
  beforeAll(() => {
  ...
  const testName =
    "PROMO4:Promo/Discount-4 Provide SHS BC add promo code (negative test) Equip60";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll PROMO-4");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll PROMO-4");
    });
...
async function stepsPromo1(dsObj, datasetindex) {
  ...
  try {
    ...
    const promoerror = {};
    promoerror.msg = CheckoutPage.getPromoError();

    TestResult.storeOutputToDataSetResult(caseResult, datasetindex, promoerror);
    expect(promoerror).not.toBeNull();
    expect(promoerror).not.toBeUndefined();
