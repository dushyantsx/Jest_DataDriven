...
async function stepsProvide3(dsObj, datasetindex) {
  ...
  try {
    ...
    const res = await du.getCustomerIdFromCustomerName(
      dbcfg,
      customerfirstname
    );
    ...
    await SelectServicesPage.clickViewMore();

    const availsAndSelectedplan = await SelectServicesPage.selectPlan(
    ...
    TestResult.storeOutputToDataSetResult(
      caseResult,
      datasetindex,
      availsAndSelectedplan
    );

    await SelectServicesPage.enterHomeSecurityAmount("50");
    ...
    if (isValEnabled) {
      ...
      expect(
        StringUtils.replaceAll(
          JSON.stringify(youpickequipments),
          "[^a-zA-Z_]",
          ""
        )
      ).toContain(
        StringUtils.replaceAll(expectedypequipments, "[^a-zA-Z_]", "")
      );
    }
    ...
    if (isValEnabled) {
      ...
      expect(
        ...
        StringUtils.replaceAll(exphsaddiontalequipments, "[^a-zA-Z_]", "")
      );
    }
    ...
    for (let orIndex = 0; orIndex < pendingWorkOrders.length; orIndex++) {
      ...
      var workOrderNumber = pendingWorkOrders[orIndex][0];
    ...
    adcid = StringUtils.substringBetween(test, "<int>", "</int>");
    logger.info(adcid);
    ...
    if (isValEnabled) {
      ...
      const appointmentenddate =
        parseddst.Body.getWorkOrderResponse.workOrder.appointmentEndDate;
