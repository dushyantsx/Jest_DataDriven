...
const testId = TestIdsMap.promo6TestID;
 ...
 */
let caseResult;
describe("Promo6: SHS and LW Test Scenarios", () => {
  beforeAll(() => {
  ...
  const testName = "Promo-6: Add Promo via discounts";
  describe(testName, () => {
    beforeAll(() => {
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot === "Y") {
          return await stepsPromo6(dsObj, index)
            .then((v) => {
...
async function stepsPromo6(dsObj, datasetindex) {
  ...
  try {
    ...
    const provide8plan = config.getFirstTestDataAssetResultsForGivenTestId(
      TestIdsMap.provide8SSHLWTech
    ).request.shsserviceplan;

    const city = config.getFirstTestDataAssetResultsForGivenTestId(
      TestIdsMap.move5TestId
    ).request.shsserviceplan;

    await CsrDesktopPage.searchCustomerForEmail(provide8Email);

    await CsrDesktopPage.clickCityRadioButton(city);

    await CsrDesktopPage.clickOnManageDiscount();

    await sel.getJsUtils().isPageLoaded();

    await CheckoutPage.selectServiceForDiscount("Smart Automation Plus Video"); //provide8plan);
