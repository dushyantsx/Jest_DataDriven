...
const testId = TestIdsMap.provide2TestId;
...
describe("Provide2: SHS & LW Test Scenarios", () => {
  ...
  let testName = "Provide-2: Provide SHS; Product: Home Security 60 Months; Techniocian Install";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsProvide2(dsObj, datasetindex) {
  ...
  try {
    ...
    if (StringUtils.containsIgnoreCase(sPlan, "secure") == false) {
      let youpickequipments = await EquipmentPage.completeYouPickSectionOnDefaults();
      logger.result(`Default equipments are selected ${JSON.stringify(youpickequipments)} `);
    ...
    let addOnAdditionalEquipments = await EquipmentPage.completeAddOnEquipmentSectionOnDefaults(deliveryMethod, true);
    ...
    if (StringUtils.containsIgnoreCase(sPlan, "secure") && StringUtils.equalsIgnoreCase(deliveryMethod, "tech")) {
