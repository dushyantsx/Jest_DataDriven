...
describe("SHS and LW Test Scenarios - Change2", () => {
  ...
  let testName = "Change-2: Remove Equipment";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsChange2(dsObj, datasetindex) {
  ...
  try {
    ...
    await EquipmentPage.clickOnMinusEquipment(dr.getDataSetKeyValue(dsObj, "input", "equipmenttoadd"));
    let deliverymethod = "Tech";
    equipments = await EquipmentPage.completeHomeSecuritySectionOnDefaults(deliverymethod, true);
    allequipments.push(equipments);
    storeOutputToDataSetResult(caseResult, datasetindex, equipments);
    isValEnabled = tval.isValidationEnabled(valFile, testId, "validate-default-equipments");
    if (isValEnabled) {
