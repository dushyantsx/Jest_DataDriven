...
const { clickNextButton } = require("../../../../src/pages/home-security/SelectServicesPage");
...
describe("SHS and LW Test Scenarios", () => {
  ...
  let testName = "Other-Products-02: HS and Optik TV to already existing connection";
  describe(testName, () => {
    beforeAll(() => {
...
async function stepsOther2(dsObj, datasetindex) {
  ...
  try {
    ...
    await CsrDesktopPage.searchCustomerForEmail("provide_one_3216899@telus.com");

    await CsrDesktopPage.clickOnServiceslink();

    await AccountPage.clickOnAddService("Optik TV");

    let allAvailsAndSelected = [];

    let tvservice = dr.getDataSetKeyValue(dsObj, "input", "tvservices");
    let tvcommitments = dr.getDataSetKeyValue(dsObj, "input", "tvcommitments");
    let tvproducttype = dr.getDataSetKeyValue(dsObj, "input", "tvproducttype");
    let tvproductpackage = dr.getDataSetKeyValue(dsObj, "input", "tvproductpackage");
    let tvpackagesecondary = dr.getDataSetKeyValue(dsObj, "input", "tvpackagesecondary");
    ...
    storeOutputToDataSetResult(caseResult, datasetindex, allAvailsAndSelected);
    logger.result(`There were available services ${JSON.stringify(availsAndSelected)}`);

    let internetservice = dr.getDataSetKeyValue(dsObj, "input", "internetservice");
    let internetcommitments = dr.getDataSetKeyValue(dsObj, "input", "internetcommitments");
    let internetproductoffering = dr.getDataSetKeyValue(dsObj, "input", "internetproductoffering");
    ...
    await CustomizePage.selectFeature(dr.getDataSetKeyValue(dsObj, "input", "internetfeature"));
    ...
    let internetequipments = await EquipmentPage.completeHomeSecuritySectionOnDefaults();

    storeOutputToDataSetResult(caseResult, datasetindex, internetequipments);
    ...
    let tvequipments = await EquipmentPage.completeHomeSecuritySectionOnDefaults();

    storeOutputToDataSetResult(caseResult, datasetindex, tvequipments);
    isValEnabled = tval.isValidationEnabled(valFile, testId, "validate-default-equipments");
    if (isValEnabled) {
      logger.debug(`Validating default equipments available or not`);
      expect(tvequipments).not.toBeNull();
      expect(tvequipments).not.toBeUndefined();

      expect(tvequipments.myEquipments).not.toBeNull();
      expect(tvequipments.myEquipments).not.toBeUndefined();

      expect(tvequipments.includedEquipments).not.toBeNull();
      expect(tvequipments.includedEquipments).not.toBeUndefined();

      expect(tvequipments.additionalEquipments).not.toBeNull();
      expect(tvequipments.additionalEquipments).not.toBeUndefined();
    }
