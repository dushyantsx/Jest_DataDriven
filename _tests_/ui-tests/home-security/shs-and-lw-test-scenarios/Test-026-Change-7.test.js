...
const testId = TestIdsMap.change7TestID;
...
describe("SHS and LW Test Scenarios", () => {
  ...
  let testName = "Change-7: Add Promo";
  describe(testName, () => {
    beforeAll(() => {
      logger.enterMethod("beforeAll Change-7");
      caseResult = TestResult.TestCaseResult(testId, testName);

      logger.exitMethod("beforeAll Change-7");
    });
    ...
    sheetDataAsJsonArray.forEach((singleJsonRowObject, index) => {
      test(`${testName} for data-set-${index + 1}`, async (done) => {
        ...
        if (executeOrNot == "Y") {
          return await stepsChange7(dsObj, index)
            .then((v) => {
...
async function stepsChange7(dsObj, datasetindex) {
  ...
  try {
    ...
    await CsrDesktopPage.clickOnDiscount();
    ...
    if (isValEnabled) {
      ...
      logger.debug(`Validating cost ${discount} with ${discounttovalidate}`);
      expect(StringUtils.equalsIgnoreCase(discount, discounttovalidate)).toBeTruthy();
    }
